import { APP_VERSION, uid, now, clean, dateFromId, gradeStats, terms, isFailed, emptyState, hasSecurity, setupSecurity, unlockSecurity, lockSecurity, loadState, saveState, putFile, getFile, allFiles, putFiles, previewWorkbook, exportSheets } from './data.js';
...
const studentName = id => state.students[id]?.name || id;
...
function fileName(s, type, date, seq, ext) { return `${(s.className || '未分班').replace(/[\\/:*?"<>|]/g, '_')}_${s.name.replace(/[\\/:*?"<>|]/g, '_')}_${type}_${(date || today()).replaceAll('-', '')}_${seq}.${ext}`; }
...
async function gate() {
  ...
  app.innerHTML = `<div class="lock-page"><div class="lock-card"><div class="lock-mark">档</div><h1>${exists ? '欢迎回来' : '建立你的工作台'}</h1><p>${exists ? '输入解锁密码，查看保存在这台设备上的学生资料。' : '学生资料仅保存在这台设备中。请设置解锁密码，并定期导出加密数据包备份。'}</p><form id="gate-form">${field('解锁密码', 'password', '', 'password', 'required minlength="8" autocomplete="current-password"')}${exists ? '' : field('再次输入密码', 'confirm', '', 'password', 'required minlength="8" autocomplete="new-password"')}<button class="btn" type="submit">${exists ? '解锁' : '创建并进入'}</button></form><div class="lock-foot">忘记密码无法解密本机资料。请妥善保存密码和导出的加密备份包。</div></div></div>`;
...
function shell(body) {
  const labels = { home: '工作概览', students: '学生档案', alerts: '预警与待办', data: '数据管理' };
  app.innerHTML = `<div class="shell"><header class="topbar"><div class="toprow"><div><div class="eyebrow">COUNSELOR WORKBENCH · v${APP_VERSION}</div><div class="brand">辅导员工作台</div></div><button class="icon-btn" data-action="lock" aria-label="锁定">⌁</button></div><p class="subtitle">${labels[view] || '学生档案'} · 资料保存在此设备</p></header><main class="content">${body}</main><nav class="tabbar" aria-label="主导航">${[['home','⌂','首页'],['students','▤','学生'],['alerts','◷','提醒'],['data','⇅','数据']].map(([key, icon, label]) => `<button data-view="${key}" class="${view === key ? 'active' : ''}"><span class="tab-icon">${icon}</span><span>${label}</span></button>`).join('')}</nav></div>`;
...
function homeView() {
  ...
  return `<div class="hero"><div class="kicker">今天的工作概览</div><strong>${total} 名学生</strong><p>随时查询、记录与跟进</p></div><div class="stats"><button class="stat" data-view="students"><strong>${total}</strong><span>学生档案</span></button><button class="stat warn" data-view="alerts"><strong>${warnings.length}</strong><span>学业预警</span></button><button class="stat due" data-view="alerts"><strong>${due.length}</strong><span>到期待办</span></button></div><div class="section-title"><h2>优先跟进</h2><small>${selectedTerm ? esc(selectedTerm) : '尚无成绩数据'}</small></div><div class="panel list">${due.length ? due.slice(0, 4).map(r => `<button class="list-row" data-student="${esc(r.studentId)}"><div class="avatar">◷</div><div class="row-main"><strong>${esc(studentName(r.studentId))} · ${esc(r.todo)}</strong><small>到期 ${fmtDate(r.dueDate)} · ${esc(r.type)}</small></div><span class="badge red">待办</span></button>`).join('') : warnings.slice(0, 4).map(id => `<button class="list-row" data-student="${esc(id)}"><div class="avatar">${esc(state.students[id].name.slice(0, 1))}</div><div class="row-main"><strong>${esc(state.students[id].name)} · 本学期挂科 ${gradeStats(state,id,selectedTerm).semester} 门</strong><small>${esc(state.students[id].className)} · ${esc(id)}</small></div><span class="badge">预警</span></button>`).join('') || empty('暂无待跟进事项', '导入成绩或添加带到期日期的工作记录后，会出现在这里。')}</div><div class="section-title"><h2>最近记录</h2><small>${state.records.length} 条</small></div><div class="panel list">${recent.length ? recent.map(r => `<button class="list-row" data-student="${esc(r.studentId)}"><div class="avatar">✎</div><div class="row-main"><strong>${esc(studentName(r.studentId))} · ${esc(r.type)}</strong><small>${fmtDate(r.date)} · ${esc(r.summary || '无摘要')}</small></div><span class="chevron">›</span></button>`).join('') : empty('还没有工作记录', '在学生档案中添加谈话或家校联系记录。')}</div>`;
}
function empty(title, body) { return `<div class="empty"><span class="emoji">◌</span><strong>${esc(title)}</strong><p>${esc(body)}</p></div>`; }
function studentsView() {
  if (selectedId && student()) return detailView();
  const query = search.toLowerCase(), filtered = Object.values(state.students).filter(s => [s.id,s.name,s.className,s.major,s.dorm].some(v => clean(v).toLowerCase().includes(query))).sort((a,b) => a.className.localeCompare(b.className,'zh-CN') || a.name.localeCompare(b.name,'zh-CN'));
  return `<div class="toolbar"><h2>学生档案 <span class="muted tiny">${filtered.length}</span></h2><button class="btn small" data-action="add-student">＋ 新增</button></div><div class="search-wrap"><input class="search" id="student-search" type="search" placeholder="搜索姓名、学号、班级、专业" value="${esc(search)}"></div><div class="panel list">${filtered.length ? filtered.map(s => `<button class="list-row" data-student="${esc(s.id)}"><div class="avatar">${esc(s.name.slice(0,1))}</div><div class="row-main"><strong>${esc(s.name)}</strong><small>${esc(s.className || '未分班')} · ${esc(s.id)} · ${esc(s.major)}</small></div><span class="chevron">›</span></button>`).join('') : empty(search ? '没有匹配的学生' : '还没有学生档案', search ? '试试学号或班级关键词。' : '可手工新增，或从固定 Excel 模板批量导入。')}</div>`;
}
...
function detailView() {
  ...
  let body = '';
  if (detailTab === 'info') {
    const fields = [['学号',s.id],['姓名',s.name],['班级',s.className],['专业',s.major],['宿舍',s.dorm],['本人电话',s.phone],['家长电话',s.parentPhone],['家庭住址',s.address],['性别',s.gender],['出生日期',s.birthDate]];
  ...
  return `<button class="back" data-action="back-students">← 返回学生列表</button><div class="panel detail-head"><div class="avatar" id="detail-avatar">${esc(s.name.slice(0,1))}</div><div><h2>${esc(s.name)}</h2><p>${esc(s.className || '未分班')} · ${esc(s.major || '未填专业')}</p><span class="pill">学号 ${esc(s.id)}</span></div></div><div class="segmented">${tabs.map(([key,label]) => `<button data-detail-tab="${key}" class="${detailTab === key ? 'active' : ''}">${label}</button>`).join('')}</div>${body}`;
}
...
function alertsView() {
  ...
  return `<div class="toolbar"><h2>预警与待办</h2><button class="btn small secondary" data-action="threshold">设置规则</button></div><div class="notice">本学期挂科达到 <strong>${threshold} 门</strong>时提示；累计挂科按曾经挂科的不同课程统计，补考或重修通过后仍保留历史。</div><div class="section-title"><h2>学业预警</h2><select id="term-select" class="mini-select"><option value="">选择学期</option>${terms(state).map(term => `<option value="${esc(term)}" ${term === selectedTerm ? 'selected' : ''}>${esc(term)}</option>`).join('')}</select></div><div class="panel list">${warn.length ? warn.map(({s,n}) => `<button class="list-row" data-student="${esc(s.id)}"><div class="avatar">${esc(s.name.slice(0,1))}</div><div class="row-main"><strong>${esc(s.name)}</strong><small>${esc(s.className)} · 历史累计 ${gradeStats(state,s.id,selectedTerm).cumulative} 门</small></div><span class="badge red">${n} 门</span></button>`).join('') : empty('暂无符合规则的学生', selectedTerm ? '可调整挂科门数阈值。' : '先导入或录入成绩，并选择学期。')}</div><div class="section-title"><h2>后续待办</h2><small>${due.length} 项</small></div><div class="panel list">${due.length ? due.map(r => `<button class="list-row" data-student="${esc(r.studentId)}"><div class="avatar">◷</div><div class="row-main"><strong>${esc(studentName(r.studentId))} · ${esc(r.todo)}</strong><small>${r.dueDate ? `到期 ${fmtDate(r.dueDate)}` : '未设到期日'} · ${esc(r.type)}</small></div><span class="badge ${r.dueDate && r.dueDate <= today() ? 'red' : ''}">${r.dueDate && r.dueDate <= today() ? '已到期' : '待办'}</span></button>`).join('') : empty('暂无待办', '工作记录中添加后续待办和到期日期。')}</div>`;
}
function dataView() {
  return `<div class="toolbar"><h2>数据管理</h2><span class="pill">仅本机</span></div><div class="notice warn">这台 iPhone 的数据不会自动出现在电脑上。请定期导出加密数据包；卸载应用或清除网站数据可能导致本机资料丢失。</div><div class="section-title"><h2>Excel 模板</h2></div><div class="panel"><div class="file-card"><h3>空白固定模板</h3><p>包含学生、成绩、资助、工作记录四张表。学生表可增加自定义列。</p><button class="btn secondary" data-action="template">下载空白模板</button></div><div class="file-card"><h3>导入 Excel</h3><p>先预览新增、更新和错误行，再确认导入。</p><button class="btn secondary" data-action="import-xlsx">选择 Excel 文件</button></div><div class="file-card"><h3>导出当前数据</h3><p>导出四张表；记录 ID 保留，方便修改后再次导入。</p><button class="btn secondary" data-action="export-xlsx">导出 Excel</button></div></div><div class="section-title"><h2>照片与传输</h2></div><div class="panel"><div class="file-card"><h3>证件照批量导入</h3><p>照片按“学号.jpg / 学号.png”命名后打包。建议每个 ZIP 不超过 50 MB，可分批导入。</p><button class="btn ghost" data-action="import-photos">选择照片 ZIP</button></div><div class="file-card"><h3>导出加密数据包</h3><p>包含档案、工作记录和附件。文件名带版本号与导出日期时间。</p><button class="btn" data-action="export-package">生成传输包 / 备份</button></div><div class="file-card"><h3>导入加密数据包</h3><p>按记录 ID 合并；较新的记录覆盖较旧版本。</p><button class="btn ghost" data-action="import-package">选择数据包 ZIP</button></div></div><div class="section-title"><h2>使用说明</h2></div><div class="panel pad"><p class="tiny muted" style="line-height:1.7;margin:0">iPhone Safari 打开应用网址后，使用分享菜单中的“添加到主屏幕”。首次载入后可在本机使用。数据与附件使用解锁密码加密保存在此设备；导出的 ZIP 用你设置的传输密码再次加密。心理信息不在第一版范围内。</p></div>`;
}
function render() {
...
function studentForm(existing) {
  const s = existing || {};
  showModal(existing ? '编辑学生档案' : '新增学生', `${field('学号 *','id',s.id,'text',existing ? 'readonly required' : 'required')}${field('姓名 *','name',s.name,'text','required')}${field('班级','className',s.className)}${field('专业','major',s.major)}${field('宿舍','dorm',s.dorm)}${field('本人电话','phone',s.phone,'tel')}${field('家长电话','parentPhone',s.parentPhone,'tel')}${area('家庭住址','address',s.address)}${field('身份证号','idNumber',s.idNumber)}${choose('性别','gender',['','男','女','其他'],s.gender)}${field('出生日期','birthDate',s.birthDate,'date')}${area('自定义字段（每行：字段名=内容）','custom',Object.entries(s.custom || {}).map(([k,v]) => `${k}=${v}`).join('\n'))}<p class="inline-note">有效的 18 位身份证号可自动生成出生日期；也可手工填写。</p>`, '保存档案', async form => {
    const id = clean(form.get('id')), name = clean(form.get('name')); if (!id || !name) throw new Error('学号和姓名必填');
    ...
    state.students[id] = { ...s, id, name, className: clean(form.get('className')), major: clean(form.get('major')), dorm: clean(form.get('dorm')), phone: clean(form.get('phone')), parentPhone: clean(form.get('parentPhone')), address: clean(form.get('address')), idNumber, gender: clean(form.get('gender')), birthDate: clean(form.get('birthDate')) || dateFromId(idNumber), custom, updatedAt: now() };
...
function recordForm(existing) {
  ...
  showModal(existing ? '编辑工作记录' : '添加工作记录', `${choose('类型','type',['谈话','家校联系'],r.type)}${field('日期','date',r.date || today(),'date','required')}${field('对象','subject',r.subject,'text','placeholder="例如 学生本人 / 母亲"')}${area('摘要 *','summary',r.summary)}${area('后续待办','todo',r.todo)}${field('到期日期','dueDate',r.dueDate,'date')}${field('追加截图（可多选）','images','','file','accept="image/*" multiple')}`, '保存记录', async (form,data) => {
    ...
    const next = { ...r, id: r.id || id, studentId: s.id, type, date, subject: clean(form.get('subject')), summary, todo: clean(form.get('todo')), dueDate: clean(form.get('dueDate')), done: r.done || false, attachments, updatedAt: now() };
